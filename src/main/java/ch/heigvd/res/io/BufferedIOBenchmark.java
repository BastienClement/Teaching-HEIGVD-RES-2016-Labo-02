...
public class BufferedIOBenchmark {
	...
	/**
	 * Describe the type of IO tested
	 */
	public enum IOType {
		READ,
		WRITE
	...
	public class TestResult {
		...
		public final IOStrategy strategy;
		public final int blockSize;
		public final long fileSize;
		public final long duration;

		public TestResult(IOType type, IOStrategy strategy, int blockSize, long fileSize, long duration) {
			this.type = type;
	...
	public static class TestOutputBuilder {
		OutputStreamWriter osw;

		public TestOutputBuilder(OutputStream os) {
			this.osw = new OutputStreamWriter(os);
		}

		public void headers(String ...headers) throws IOException {
			osw.write(String.join(",", (CharSequence[]) headers));
			osw.write("\n");
		}

		public void log(TestResult tr) throws IOException {
			osw.write(String.format("%s,%s,%d,%d,%d\n",
					tr.type.name(),
					tr.strategy.name(),
					tr.blockSize,
					tr.fileSize,
	...
	private TestResult produceTestData(IOStrategy ioStrategy, long numberOfBytesToWrite, int blockSize) {
		...
		return new TestResult(IOType.WRITE, ioStrategy, blockSize, numberOfBytesToWrite, time);
	}
	...
	private TestResult consumeTestData(IOStrategy ioStrategy, int blockSize) {
		...
		long time = Timer.takeTime();
		LOG.log(Level.INFO, "  > Done in {0} ms.", time);
		return new TestResult(IOType.READ, ioStrategy, blockSize, bytesRead, time);
	}
	...
	public static void main(String[] args) throws IOException {
		...
		TestOutputBuilder tob = new TestOutputBuilder(new BufferedOutputStream(new FileOutputStream("output.csv")));
		tob.headers("operation", "strategy", "blockSize", "fileSizeInBytes", "durationInMs");

		LOG.log(Level.INFO, "");
		LOG.log(Level.INFO, "*** BENCHMARKING WRITE OPERATIONS (with BufferedStream)", Timer.takeTime());
		tob.log(bm.produceTestData(IOStrategy.BlockByBlockWithBufferedStream, NUMBER_OF_BYTES_TO_WRITE, 500));
		tob.log(bm.produceTestData(IOStrategy.BlockByBlockWithBufferedStream, NUMBER_OF_BYTES_TO_WRITE, 50));
		tob.log(bm.produceTestData(IOStrategy.BlockByBlockWithBufferedStream, NUMBER_OF_BYTES_TO_WRITE, 5));
		tob.log(bm.produceTestData(IOStrategy.ByteByByteWithBufferedStream, NUMBER_OF_BYTES_TO_WRITE, 0));

		LOG.log(Level.INFO, "");
		LOG.log(Level.INFO, "*** BENCHMARKING WRITE OPERATIONS (without BufferedStream)", Timer.takeTime());
		tob.log(bm.produceTestData(IOStrategy.BlockByBlockWithoutBufferedStream, NUMBER_OF_BYTES_TO_WRITE, 500));
		tob.log(bm.produceTestData(IOStrategy.BlockByBlockWithoutBufferedStream, NUMBER_OF_BYTES_TO_WRITE, 50));
		tob.log(bm.produceTestData(IOStrategy.BlockByBlockWithoutBufferedStream, NUMBER_OF_BYTES_TO_WRITE, 5));
		tob.log(bm.produceTestData(IOStrategy.ByteByByteWithoutBufferedStream, NUMBER_OF_BYTES_TO_WRITE, 0));

		LOG.log(Level.INFO, "");
		LOG.log(Level.INFO, "*** BENCHMARKING READ OPERATIONS (with BufferedStream)", Timer.takeTime());
		tob.log(bm.consumeTestData(IOStrategy.BlockByBlockWithBufferedStream, 500));
		tob.log(bm.consumeTestData(IOStrategy.BlockByBlockWithBufferedStream, 50));
		tob.log(bm.consumeTestData(IOStrategy.BlockByBlockWithBufferedStream, 5));
		tob.log(bm.consumeTestData(IOStrategy.ByteByByteWithBufferedStream, 0));

		LOG.log(Level.INFO, "");
		LOG.log(Level.INFO, "*** BENCHMARKING READ OPERATIONS (without BufferedStream)", Timer.takeTime());
		tob.log(bm.consumeTestData(IOStrategy.BlockByBlockWithoutBufferedStream, 500));
		tob.log(bm.consumeTestData(IOStrategy.BlockByBlockWithoutBufferedStream, 50));
		tob.log(bm.consumeTestData(IOStrategy.BlockByBlockWithoutBufferedStream, 5));
		tob.log(bm.consumeTestData(IOStrategy.ByteByByteWithoutBufferedStream, 0));
